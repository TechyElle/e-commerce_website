import { Cpu, Target, Zap } from 'lucide-react';
import logoQrImg from '../../imports/Logo & QR/LOGO QR.png';

export function About() {
  return (
    <div className="min-h-screen bg-[#111111]">
      {/* Hero Section */}
      <section className="py-20 bg-[#1e1e1e] border-b border-[rgba(255,255,255,0.1)]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h1 className="text-5xl md:text-6xl mb-6 text-center" style={{ fontFamily: 'Orbitron, sans-serif', fontWeight: 900 }}>
            ABOUT <span className="text-[#00BFDF]">XONTRIX</span>
          </h1>
          <p className="text-xl text-center text-[#aaaaaa] max-w-3xl mx-auto" style={{ fontFamily: 'Rajdhani, sans-serif' }}>
            Redefining the electronics e-commerce experience through cutting-edge technology and AI-powered innovation.
          </p>
        </div>
      </section>

      {/* Main Content - Two Column */}
      <section className="py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            {/* Left Column - Brand Story */}
            <div className="lg:col-span-2 space-y-8">
              <div className="border border-[rgba(255,255,255,0.1)] p-8 bg-[#1e1e1e]">
                <h2 className="text-3xl mb-4" style={{ fontFamily: 'Orbitron, sans-serif', fontWeight: 700 }}>
                  OUR MISSION
                </h2>
                <p className="text-[#aaaaaa] mb-4" style={{ fontFamily: 'Rajdhani, sans-serif', fontSize: '1.125rem' }}>
                  At Xontrix, we believe technology should be accessible, innovative, and future-forward. We're not just an electronics store — we're a technology platform that combines cutting-edge products with AI-powered business intelligence.
                </p>
                <p className="text-[#aaaaaa]" style={{ fontFamily: 'Rajdhani, sans-serif', fontSize: '1.125rem' }}>
                  From smartphones to smart home devices, from laptops to wearables, we curate only the best technology products to empower your digital lifestyle.
                </p>
              </div>

              {/* Values */}
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="border border-[rgba(255,255,255,0.1)] p-6 bg-[#1e1e1e] hover:border-[#00BFDF] transition-all">
                  <Cpu className="w-10 h-10 text-[#00BFDF] mb-4" />
                  <h3 className="text-lg mb-2" style={{ fontFamily: 'Orbitron, sans-serif', fontWeight: 600 }}>
                    INNOVATION
                  </h3>
                  <p className="text-[#aaaaaa] text-sm" style={{ fontFamily: 'Rajdhani, sans-serif' }}>
                    Always pushing the boundaries of what's possible in tech retail.
                  </p>
                </div>
                <div className="border border-[rgba(255,255,255,0.1)] p-6 bg-[#1e1e1e] hover:border-[#00BFDF] transition-all">
                  <Target className="w-10 h-10 text-[#00BFDF] mb-4" />
                  <h3 className="text-lg mb-2" style={{ fontFamily: 'Orbitron, sans-serif', fontWeight: 600 }}>
                    PRECISION
                  </h3>
                  <p className="text-[#aaaaaa] text-sm" style={{ fontFamily: 'Rajdhani, sans-serif' }}>
                    Curated selection of only the highest-quality products.
                  </p>
                </div>
                <div className="border border-[rgba(255,255,255,0.1)] p-6 bg-[#1e1e1e] hover:border-[#00BFDF] transition-all">
                  <Zap className="w-10 h-10 text-[#00BFDF] mb-4" />
                  <h3 className="text-lg mb-2" style={{ fontFamily: 'Orbitron, sans-serif', fontWeight: 600 }}>
                    SPEED
                  </h3>
                  <p className="text-[#aaaaaa] text-sm" style={{ fontFamily: 'Rajdhani, sans-serif' }}>
                    Lightning-fast delivery and seamless shopping experience.
                  </p>
                </div>
              </div>

              {/* Stats */}
              <div className="grid grid-cols-3 gap-6 border border-[rgba(255,255,255,0.1)] p-8 bg-[#1e1e1e]">
                <div className="text-center">
                  <div className="text-4xl text-[#00BFDF] mb-2" style={{ fontFamily: 'Orbitron, sans-serif', fontWeight: 900 }}>
                    10K+
                  </div>
                  <div className="text-[#aaaaaa] text-sm" style={{ fontFamily: 'Rajdhani, sans-serif' }}>
                    Products
                  </div>
                </div>
                <div className="text-center">
                  <div className="text-4xl text-[#00BFDF] mb-2" style={{ fontFamily: 'Orbitron, sans-serif', fontWeight: 900 }}>
                    50K+
                  </div>
                  <div className="text-[#aaaaaa] text-sm" style={{ fontFamily: 'Rajdhani, sans-serif' }}>
                    Customers
                  </div>
                </div>
                <div className="text-center">
                  <div className="text-4xl text-[#00BFDF] mb-2" style={{ fontFamily: 'Orbitron, sans-serif', fontWeight: 900 }}>
                    99%
                  </div>
                  <div className="text-[#aaaaaa] text-sm" style={{ fontFamily: 'Rajdhani, sans-serif' }}>
                    Satisfaction
                  </div>
                </div>
              </div>
            </div>

            {/* Right Column - QR Code Card */}
            <div className="lg:col-span-1">
              <div className="bg-[#1e1e1e] border border-[rgba(255,255,255,0.1)] p-8 sticky top-24">
                <h3 className="text-xl mb-6 text-center" style={{ fontFamily: 'Orbitron, sans-serif', fontWeight: 700 }}>
                  CONNECT WITH US
                </h3>
                <div className="bg-[#111111] p-6 mb-6">
                  <img src={logoQrImg} alt="Scan to Connect" className="w-full h-auto" />
                </div>
                <p className="text-[#00BFDF] text-sm text-center mb-4" style={{ fontFamily: 'Orbitron, sans-serif', letterSpacing: '0.05em' }}>
                  SCAN TO ACCESS
                </p>
                <p className="text-[#aaaaaa] text-xs text-center" style={{ fontFamily: 'Rajdhani, sans-serif' }}>
                  Get instant access to our mobile app, exclusive deals, and member benefits
                </p>
              </div>
            </div>
          </div>
        </div>
      </section>
    </div>
  );
}
