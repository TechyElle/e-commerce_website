...
try {
    ...
    if ($method === 'POST') {
        ...
        $stockStmt = $pdo->prepare(
            'UPDATE products
             SET in_stock = CASE WHEN stock - :quantity1 > 0 THEN 1 ELSE 0 END,
                 stock = stock - :quantity2
             WHERE id = :product_id AND stock >= :quantity3 AND in_stock = 1'
        );
        ...
        foreach ($data['items'] as $item) {
            ...
            $stockStmt->execute([':quantity1' => $quantity, ':quantity2' => $quantity, ':quantity3' => $quantity, ':product_id' => $productId]);
